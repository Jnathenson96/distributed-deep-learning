...
if local_demo:
    ...
    #target = tvm.target.arm_cpu('arm64-v8a')
...
tmp = util.tempdir()
lib_fname = './tmp/net.tar'
lib.export_library("./tmp/deploy_lib_cpu.so", cc="/opt/android-toolchain-arm64/bin/aarch64-linux-android-g++")
with open('tmp/deploy_graph.json', 'w') as fo:
    fo.write(graph.json())
with open('tmp/deploy_param.params', 'wb') as fo:
